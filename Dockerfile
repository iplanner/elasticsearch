...
# Installiere openssh-server, bash und gosu
RUN apt-get update && \
    apt-get install -y openssh-server bash gosu && \
    apt-get clean && \
...
# Setze Shell für Benutzer elasticsearch auf /bin/bash (Render-Anforderung)
...
exec gosu elasticsearch /usr/share/elasticsearch/bin/elasticsearch' > /start.sh && \
chmod +x /start.sh

# Wechsle zurück zum Nicht-Root-Benutzer (Render-Anforderung)
USER elasticsearch
